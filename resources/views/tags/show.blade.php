...
@section('title', "Jobs tagged with {$tag->name}")
    ...
    <div class="relative mx-auto px-4">
        ...
        <div class="text-center mb-12 animate-fade-in">
            <h2 class="text-4xl font-bold bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent">
                Jobs tagged with #{{ $tag->name }}
            </h2>
            <p class="text-gray-400 mt-2">{{ $jobs->count() }} opportunities found</p>
        </div>
                        ...
                        <div class="relative h-full bg-white/[0.05] backdrop-blur-xl rounded-xl border border-white/10 p-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-white/5">
                            ...
                            <div class="flex flex-wrap gap-2 mb-6">
                                @foreach ($job->tags as $jobTag)
                                    <a href="{{ route('tags.show', $jobTag) }}" class="px-3 py-1.5 bg-white/5 text-gray-300 rounded-full text-sm border border-white/10 transition-all duration-300 hover:scale-105 hover:bg-white/10 hover:text-white">
                                        {{ $jobTag->name }}
                                    </a>
                                @endforeach
                            </div>
